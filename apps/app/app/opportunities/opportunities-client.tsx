...
function AccessBanner({ access, verification, tierName }: { access: MarketIntelligenceAccess; verification: BusinessVerificationStatus; tierName: string }) {
  ...
  if (access === "public") {
    return <div className="opportunity-access"><span><Sparkles size={18} /></span><div><strong>Public opportunity signals are open</strong><p>Browse country-product fits and demand direction. Create a free account for ranked scores and shortlists; verify the business or subscribe for evidence, barriers and action routes.</p></div><div><Link href="/sign-up">Create free account</Link><Link href="/sign-in">Sign in</Link></div></div>;
  }
      ...
      <div className="opportunity-workspace">
        ...
        <aside className="opportunity-detail" aria-live="polite">
          {selected ? <>
            ...
            </div> : <div className="opportunity-locked"><span><LockKeyhole size={21} /></span><p>{access === "public" ? "ACCOUNT + TRUST LAYERS" : "FULL ANALYSIS"}</p><h3>See the evidence and route behind this score</h3><ul><li><Check size={13} /> Buyer profiles and entry routes</li><li><Check size={13} /> Barriers and evidence requirements</li><li><Check size={13} /> Source links and recommended actions</li></ul><div>{access === "public" ? <><Link href="/sign-up">Create free account <ArrowRight size={14} /></Link><Link href="/sign-in">Sign in</Link></> : <><Link href="/verify-business">Verify business <ArrowRight size={14} /></Link><Link href="/plans">Upgrade plan</Link></>}</div></div>}
