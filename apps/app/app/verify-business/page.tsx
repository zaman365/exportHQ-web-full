...
export default async function VerifyBusinessPage({ searchParams }: { searchParams: Promise<{ draft?: string; synthetic?: string }> }) {
  const session = await requireWorkspaceFeature("home", { allowIncompleteOnboarding: true });
  const params = await searchParams;
  const isVerified = session.businessVerification === "verified";
  const isPending = session.businessVerification === "pending";
    ...
    <WorkspaceShell active="opportunities" session={session}>
      <section className="verification-page">
        ...
        {isVerified ? (
          ...
          <div className="verification-state verification-state--pending"><span><Clock3 size={25} /></span><div><p>{params.synthetic === "1" ? "SYNTHETIC DRAFT" : "DRAFT CREATED"}</p><h2>No evidence has been submitted yet</h2><span>Your company facts are prepared. The case remains a draft until the private vault has quarantined, scanned and linked an immutable evidence version. Production uploads stay disabled until those controls are activated.</span><Link href="/settings#organization">Continue preparing your profile <ArrowRight size={15} /></Link></div></div>
        ) : (
        ...
        {!isVerified && !isPending && !isDraft && <Link className="verification-later" href="/settings#organization">Not ready yet? Return to your profile and verify later <ArrowRight size={14} /></Link>}
      </section>
    </WorkspaceShell>
  );
}
