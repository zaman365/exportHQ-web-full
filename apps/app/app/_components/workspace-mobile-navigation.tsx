...
export function MobileNavigation({ active }: { active: WorkspaceDestination }) {
    ...
    <details className="mobile-navigation" ref={detailsRef}>
      ...
      <div className="mobile-navigation__panel">
        ...
        <nav aria-label="Mobile navigation">
          {workspaceGroups.map((group) => <div className="nav-group" key={group.label}><p>{group.label}</p>{group.items.map(([label, Icon, href, id]) => <Link href={href} className={active === id ? "active" : ""} key={label} onClick={close}><Icon size={17} strokeWidth={1.8} />{label}</Link>)}</div>)}
        </nav>
