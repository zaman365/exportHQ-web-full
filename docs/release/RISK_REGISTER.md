...
| --- | --- | --- | --- | --- |
| R0-R01 | Empty database cannot replay the legacy migration chain | P0 | Checksum-protected replacement baseline, local clean migrate/seed/backup/restore pass, CI gate authored | Successful protected-branch CI evidence / technical lead |
| R0-R02 | Cross-tenant access is unproved on a real application role | P0 | 29 local PostgreSQL tests pass through non-owner `NOBYPASSRLS` role; CI gate authored | Successful protected-branch CI evidence / security owner |
...
| R0-R08 | Cloudflare artifact differs from the standard Next build | P0 | Binding types, Vinext build, Wrangler dry-run and local Worker smoke pass; artifact CI/promotion workflows authored | Successful attested artifact run from protected SHA / technical lead |
